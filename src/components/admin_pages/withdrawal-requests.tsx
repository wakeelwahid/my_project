...
import { SearchIcon, FilterIcon, CheckIcon, XIcon, AlertCircle } from "lucide-react";
...
export default function WithdrawalRequests() {
  ...
  const { data: transactions, isLoading } = useQuery({
    ...
    select: (data) => data.filter((t: any) => t.type === "withdrawal"),
  });
  ...
  const hasSufficientBalance = (transaction: any) => {
    if (!users) return true; // Default to true if users data not loaded yet

    const user = users.find((u: any) => u.id === transaction.userId);
    return user && user.walletBalance >= transaction.amount;
  };

  return (
    <div className="space-y-6">
      <div className="flex justify-between">
        <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Withdrawal Requests</h1>
      </div>
      ...
      <Card>
        ...
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {filteredTransactions?.map((transaction: any) => {
                  const sufficientBalance = hasSufficientBalance(transaction);
          ...
          {statusFilter === "pending" && filteredTransactions?.length > 0 && (
            <div className="mt-6">
              <Alert variant="info">
                <AlertCircle className="h-4 w-4" />
                <AlertTitle>Important</AlertTitle>
                <AlertDescription>
                  Always verify user identity and payment details before approving withdrawals. Approved withdrawals will immediately deduct funds from the user's wallet.
                </AlertDescription>
              </Alert>
            </div>
          )}
